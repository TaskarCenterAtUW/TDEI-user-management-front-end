...
const MyDatasets = () => {
  ...
  const { mutate: publishDataset, isLoading: isPublishing } = usePublishDataset(
    { onSuccess, onError }
  );
  const { mutate: deactivateDataset, isLoading: isDeletingDataset } =
  ...
  const handlePublishDataset = (dataset) => {
    ...
    if (
      !dataset?.metadata?.dataset_detail?.valid_from ||
      !dataset?.metadata?.dataset_detail?.valid_to
    ) {
  ...
  const modalConfig = {
    release: {
      ...
      handler: () =>
        publishDataset({
          service_type: selectedDataset.data_type,
          tdei_dataset_id: selectedDataset.tdei_dataset_id,
        }),
      btnlabel: "Release",
      modaltype: "release",
    ...
    missingDates: {
      message:
        "Valid From and Valid To dates are mandatory for publishing the dataset.",
