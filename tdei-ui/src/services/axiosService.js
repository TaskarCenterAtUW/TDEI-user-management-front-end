  ...
  async (error) => {
    ...
    if ((error.status === 401 || error.response?.status === 401) && !originalRequest._retry) {
      ...
      try {
        ...
        if ((refreshError.status === 401 || refreshError.response?.status === 401) && originalRequest.url.indexOf("/authenticate") === -1 && window.location.pathname !== "/login") {
    ...
    } else if ((error.status === 401 || error.response?.status === 401) && originalRequest._retry && originalRequest.url.indexOf("/authenticate") == -1 && window.location.pathname !== "/login") {
      //----------------------------
